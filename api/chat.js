module.exports = async (req, res) => {
  ...
  try {
    ...
    const fireworksPayload = {
      model,
      messages,
      // qwen3-30b-a3b optimized parameters
    ...
    if (stream) {
      ...
      if (!response.ok) {
        ...
        // Handle specific qwen3-30b-a3b errors
        let errorMessage = errorText;
        if (response.status === 429) {
          errorMessage = 'Rate limit exceeded. qwen3-30b-a3b has usage limits.';
        } else if (response.status === 401) {
          errorMessage = 'Invalid API key or insufficient permissions for qwen3-30b-a3b.';
        } else if (response.status === 400) {
          errorMessage = 'Invalid request format for qwen3-30b-a3b. Check parameters.';
        }
      ...
      if (!response.body) {
        return res.status(500).json({ error: 'No response body from qwen3-30b-a3b API' });
      }
      ...
      try {
        ...
        console.error('Streaming error with qwen3-30b-a3b:', error);
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          errorMessage = 'qwen3-30b-a3b service temporarily unavailable. Try again later.';
        }
      ...
      // Log qwen3-30b-a3b usage stats
      if (data.usage) {
        console.log('qwen3-30b-a3b Usage:', {
    ...
    console.error('Server error with qwen3-30b-a3b:', error);
    
    // Enhanced error handling for qwen3-30b-a3b
    let errorMessage = error.message;
    if (error.message.includes('fetch')) {
      errorMessage = 'Network error connecting to qwen3-30b-a3b. Check your internet connection.';
    } else if (error.message.includes('timeout')) {
      errorMessage = 'Request timeout with qwen3-30b-a3b. The model may be processing a complex request.';
    }
